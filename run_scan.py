...
def main():
    ...
    printf("===============================================")
    # weight = torch.load('/home/server/Point-NN/Point-NN/checkpoint/best_checkpoint.pth')
    # pdb.set_trace()
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        train_out = train(net, train_loader, optimizer, criterion, device, args.model)
        
        test_out = validate(net, test_loader, criterion, device, args.model)
...
def train(net, trainloader, optimizer, criterion, device, model_name="Point-PN"):
    net.train()
    train_loss = 0
    correct = 0
    total = 0
    ...
    for batch_idx, (data, xyz, label) in enumerate(trainloader):
        ...
        if "MAE" in model_name:
            logits, restore_loss = net(data, xyz)
            loss = criterion(logits, label) + restore_loss
        else:
            logits = net(data, xyz)
...
def validate(net, testloader, criterion, device, model_name="PointMLP"):
    net.eval()
    test_loss = 0
    correct = 0
    total = 0
